...
const SpotifyCallback = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        } else if (playlistsData && Array.isArray(playlistsData.playlists)) {
        ...
        if (playlistsData && playlistsData.user) {
          setUser(playlistsData.user);
        } else {
          setUser({ display_name: 'Spotify User', images: [] });
        }

        const formattedPlaylists = playlistsList
          .filter((playlist) => playlist && playlist.id)
          .map((playlist) => {
            const imageUrl = Array.isArray(playlist.images) && playlist.images[0]
              ? playlist.images[0].url
              : playlist.image;
            const totalTracks = playlist.tracks?.total ?? playlist.trackCount ?? playlist.tracksCount ?? 0;
            ...
            return {
              ...
              tracks: {
                total: Number.isFinite(totalTracks) ? totalTracks : 0,
              },
            };
          });
        ...
        const message = error && error.message ? error.message : 'Failed to load Spotify data';
        toast.error(message);
      } finally {
        setLoading(false);
      }
  ...
  const displayName = user && typeof user.display_name === 'string' ? user.display_name : 'Spotify User';
  const profileImage = user && Array.isArray(user.images) && user.images[0] ? user.images[0].url : null;
  const selectedCount = Array.isArray(selectedPlaylists) ? selectedPlaylists.length : 0;
            ...
            <div className="bg-white/5 backdrop-blur-lg rounded-xl border border-white/10 p-6">
              <PlaylistSection
                ...
                playlists={playlists.map((playlist) => ({
                  id: playlist.id,
                  name: playlist.name,
                  image: playlist.images && playlist.images[0] ? playlist.images[0].url : undefined,
